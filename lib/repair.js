...
import { estimateRouteCny, readDshSearchTelemetry, readDshTokenUsage } from './budget.js';
import { addUsage, budgetState, campaignEpoch, recordCampaign, repairCampaignGate, shouldSendConvergeMessage } from './campaign.js';
...
function overlay(config) {
  const rows = [{ id: 'agent-default-model', config: { provider: config.repair.provider, model: config.repair.model } }];
  if (config.repair.search.enabled) rows.push({ id: 'web-search-deepseek', config: { model: config.repair.search.model } });
...
export async function repairRepository(options) {
  ...
  const routeFingerprint = sha256(JSON.stringify({
    provider: config.repair.provider,
    model: config.repair.model,
  ...
  if (previousStored?.route_fingerprint && previousStored.route_fingerprint !== routeFingerprint && gate.reset !== 'reset-budget') {
    const report = {
  ...
  try {
    ...
    const carriedCost = routeCost(carriedUsage, config);
    ...
    const firstUsage = addUsage(previous?.usage, await readDshTokenUsage(join(dshHome, 'sessions')));
    const firstCost = routeCost(firstUsage, config);
    ...
    while (true) {
      ...
      diffPolicy = await classifyRepairDiff(repoPath, changedPaths, numstat);
      try {
      ...
      const interimUsage = addUsage(previous?.usage, await readDshTokenUsage(join(dshHome, 'sessions')));
      const interimCost = routeCost(interimUsage, config);
    ...
    const cost = routeCost(usage, config);
    ...
    const cost = usage ? routeCost(usage, config) : undefined;
    const attemptsUsed = Number(previous?.attempts_used ?? 0) + modelAttempts;
    const activeMs = Number(previous?.active_ms ?? 0) + modelDurationMs + verifierDurationMs;
    const state = budgetState({ usage, estimatedCny: cost?.amount, activeMs, attemptsUsed }, config.budget, config.repair.max_attempts);
    const blockedStatus = guardianError.code === 'BLOCKED_CONTRACT' ? 'BLOCKED_CONTRACT' : modelAttempted ? 'BLOCKED' : 'BLOCKED_EXTERNAL';
    const blockedLock = recordCampaign(lock, target, {
      ...
      automatic_repair_used: modelAttempted || previous?.automatic_repair_used === true,
      attempts_used: attemptsUsed,
