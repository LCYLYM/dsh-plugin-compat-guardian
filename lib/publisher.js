...
async function closeSuperseded(repoPath, targetBranch, currentVersion, env) {
  ...
  for (const item of JSON.parse(raw || '[]')) {
    ...
    await runCommand('gh', ['pr', 'comment', String(item.number), '--body', `SUPERSEDED: a newer DSH target ${currentVersion} is now being processed. This PR is preserved and will not be rewritten.`], { cwd: repoPath, timeoutMs: 60_000, env });
...
export async function publishVerified(options) {
  ...
  if (repair) {
    await runCommand('git', ['apply', '--index', join(evidencePath, 'repair.patch')], { cwd: repoPath, timeoutMs: 60_000 });
  }
  ...
  assertRepairPaths(changedPaths, config.repair.protected_paths.filter(path => path !== '.dsh-compat.lock.json'));
  ...
  if (plan.effective !== 'direct-push') {
    ...
    if (exists) return { status: 'WAITING_EXISTING_PR', candidateVersion, branch, plan };
  }
  ...
  if (plan.effective === 'direct-push') {
    ...
    issueUrl = await output('gh', ['issue', 'create', '--title', `[DSH Guardian] campaign ${candidateVersion}`, '--body-file', bodyPath], { cwd: repoPath, timeoutMs: 60_000, env });
  } else {
    await runCommand('git', ['switch', '-c', branch], { cwd: repoPath, timeoutMs: 30_000 });
  }
  const subject = `${repair ? 'fix' : 'ci'}: support DSH ${candidateVersion}`;
  ...
  if (plan.effective === 'direct-push') {
    ...
    return { status: 'PUSHED', candidateVersion, issueUrl, plan, superseded };
  }
