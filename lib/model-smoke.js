    ...
    while (attempts < 2 && !accepted) {
      ...
      try {
        ...
        await rpc(baseUrl, 'session.prompt', {
          ...
          content: [{ type: 'text', text: contract.model_smoke.prompt }, ...fixtures.map(item => ({ type: 'image', mediaType: item.mediaType, data: item.data, name: item.name }))],
        });
        ...
        accepted = { sessionId, eventTypes: [...types].sort(), finalResponseSha256: sha256(finalText(events)), imageObserved: true };
      } catch (error) {
        lastError = error;
