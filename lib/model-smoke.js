...
import { accumulateRouteCny, readDshTokenUsage } from './budget.js';
import { addUsage, budgetState, modelInputUnchangedAfterPush, recordCampaign } from './campaign.js';
import { loadConfig, loadContract, resolvePluginPaths } from './config.js';
import { dshRouteEnvironment, dshRouteRows, routeFingerprint } from './dsh-route.js';
...
import { readLock, writeReport } from './report.js';
...
async function waitReady(url, service, cwd, secretValues = []) {
  for (let attempt = 0; attempt < 240; attempt += 1) {
    if (service.exited) {
      const snapshot = service.snapshot(secretValues);
      const detail = snapshot.stderr.trim() || snapshot.stdout.trim() || snapshot.spawnError || `exit ${snapshot.exitCode ?? snapshot.signal ?? 'unknown'}`;
      throw new GuardianError('WEB_EXITED', `candidate model smoke web process exited: ${detail}`);
    }
    try {
      return await rpc(url, 'session.create', { cwd });
    } catch {}
...
export async function resolveModelFixtures(repoPath, contract) {
  ...
  if (contract.model_smoke?.input_mode === 'text') return [];
...
function overlay(config, sessions) {
  return `${stringify(dshRouteRows(config))}- id: session-persistence-jsonl\n  config:\n    root: ${JSON.stringify(sessions)}\n    compression: none\n`;
}

export function modelSmokeWebArgs(profile, overlayPath, port) {
  return ['--profile', profile, '--patch', overlayPath, '--host', '127.0.0.1', '--port', String(port)];
}

export function finalText(events) {
  const assistants = events.filter(event => event.type === 'assistant/message');
  const data = assistants.at(-1)?.data;
...
export async function runCandidateModelSmoke(options) {
  ...
  // The verifier artifact contains a provisional candidate `verified`. A failed
  // model smoke must preserve the repository's last durable verified baseline.
  const durableLock = await readLock(resolve(repoPath, '.dsh-compat.lock.json'));
  const previous = baseLock.campaigns?.[options.dshVersion];
  const inputFingerprint = sha256(JSON.stringify({
    routeFingerprint: routeFingerprint(config),
    candidateVersion: options.dshVersion,
    verifiedSnapshot: baseLock.verified?.snapshot_key ?? null,
    contract,
  }));
  ...
  if (String(previous?.status ?? '').startsWith('BLOCKED')
    && !['workflow_dispatch', 'push'].includes(process.env.GUARDIAN_TRIGGER ?? 'local')) {
    const report = { schema: 1, status: 'FROZEN', kind: 'model-smoke', candidate: { version: options.dshVersion }, error: { code: 'MODEL_RECOVERY_SIGNAL_REQUIRED', message: '同版本模型失败已冻结；schedule 不会再调模型。请手工运行或修改相关配置。' } };
    await writeReport(report, outputDirectory);
    return report;
  }
  if (modelInputUnchangedAfterPush(previous, inputFingerprint, process.env.GUARDIAN_TRIGGER)) {
    const report = { schema: 1, status: 'FROZEN', kind: 'model-smoke', candidate: { version: options.dshVersion }, error: { code: 'MODEL_INPUT_UNCHANGED', message: '本次 push 没有改变模型路由、已验证快照或 smoke contract，因此不再调模型。' } };
    await writeReport(report, outputDirectory);
    return report;
  }
  if (contract.requires_model_turn === true && contract.model_turn_scope === 'differential' && options.singleRun !== true) {
    ...
    try {
      ...
      const estimatedCny = accumulateRouteCny(previous?.estimated_cny, usage, config.pricing, {
      ...
      const rawError = error instanceof GuardianError ? error : new GuardianError('MODEL_SMOKE_UNEXPECTED', String(error));
      const failure = classifyModelFailure(rawError);
      const guardianError = new GuardianError(failure.code, failure.message);
      const blockedReport = { schema: 1, status: failure.status, kind: 'model-smoke', candidate: { version: options.dshVersion }, error: { code: guardianError.code, message: guardianError.message } };
      const blockedLock = recordCampaign(durableLock, options.dshVersion, {
  ...
  try {
    ...
    const packed = await runCommand('npm', ['pack', '--json', '--pack-destination', pack], { cwd: pluginPaths.workspace, timeoutMs: 10 * 60_000, env: { ...process.env, CI: 'true' } });
    const tarball = resolve(pack, JSON.parse(packed.stdout.slice(packed.stdout.indexOf('['), packed.stdout.lastIndexOf(']') + 1))[0].filename);
    const env = {
      ...dshRouteEnvironment(config, key), CI: 'true', DSH_HOME: dshHome,
    ...
    service = startService(dsh, modelSmokeWebArgs(config.plugin.profile, overlayPath, port), { cwd: repoPath, env, secretValues: [key] });
    let readySession = await waitReady(baseUrl, service, repoPath, [key]);
    let accepted;
    let events = [];
    let lastError;
    while (attempts < 1 && !accepted) {
      attempts += 1;
      try {
        const { sessionId } = readySession ?? await rpc(baseUrl, 'session.create', { cwd: repoPath });
        ...
        const pluginUpload = fixtures.length > 0
          ? await uploadThroughPlugin(baseUrl, sessionId, fixtures[0], contract)
          : { observed: false, promptSuffix: '' };
        ...
        if (terminal) {
          const terminalError = new GuardianError('MODEL_PROVIDER_RESPONSE', JSON.stringify(terminal.data));
          const terminalFailure = classifyModelFailure(terminalError);
          if (terminalFailure.originalCode !== terminalFailure.code) {
            throw new GuardianError(terminalFailure.code, terminalFailure.message);
          }
        }
        ...
        if (fixtures.length > 0 && !hasImage) {
          throw new GuardianError('MODEL_SMOKE_IMAGE_MISSING', 'DSH history did not persist an image attachment reference');
        }
        ...
        accepted = {
          ...
          finalResponseSha256: sha256(finalText(events)),
          imageObserved: fixtures.length > 0 ? hasImage : false,
          pluginInputObserved: pluginUpload.observed || contract.model_smoke.plugin_upload === undefined,
        };
      } catch (error) {
        lastError = error;
        const failure = classifyModelFailure(error);
        throw new GuardianError(failure.code, failure.message);
      }
    }
    ...
    const failure = classifyModelFailure(rawError);
    const guardianError = new GuardianError(failure.code, failure.message);
    if (!observedUsage && sessions) {
      try {
        observedUsage = await readDshTokenUsage(sessions);
      } catch {
        // Provider failures can happen before the first session ledger is flushed.
      }
    }
    ...
    const blockedLock = recordCampaign(durableLock, options.dshVersion, {
      ...
      status: failure.status,
      input_fingerprint: inputFingerprint,
      route_fingerprint: routeFingerprint(config),
      candidate_smoke: { status: blockedReport.status, attempts, error: blockedReport.error },
