...
export function startService(command, args = [], options = {}) {
  ...
  return {
    ...
    // Some child protocols announce one-time credentials on stdout. Consumers
    // may inspect this transient buffer to complete the protocol, but reports
    // and error snapshots must continue to use snapshot(), which redacts it.
    protocolStdout() {
      return stdout;
    },
