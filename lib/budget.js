...
export function accumulateRouteCny(previous, usageDelta, pricing, route, date = new Date()) {
  const delta = estimateRouteCny(usageDelta, pricing, route, date);
  if (previous === undefined || previous === null) return delta;
  const previousKnown = previous.known ?? Number.isFinite(previous.amount);
  if (!previousKnown || !delta.known) {
    return {
      amount: null,
      band: previous.band === delta.band ? delta.band : 'mixed',
      revision: previous.revision === delta.revision ? delta.revision : 'mixed',
      known: false,
    };
  }
  return {
    amount: Number((Number(previous.amount) + Number(delta.amount)).toFixed(6)),
    band: previous.band === delta.band ? delta.band : 'mixed',
    revision: previous.revision === delta.revision ? delta.revision : 'mixed',
    known: true,
  };
