...
import { redactText, runCommand, shellCommand, startService } from './process.js';
...
export function sanitizedVerifierDiagnostic(details, replacements = []) {
  if (details === null || typeof details !== 'object' || Array.isArray(details)) return undefined;
  const raw = [details.stderr, details.spawnError, details.stdout]
    .filter(value => typeof value === 'string' && value.trim() !== '')
    .join('\n');
  if (raw === '') return undefined;
  const diagnostic = sanitizeOutput(redactText(raw), replacements)
    .replace(/(?:\/home\/runner\/work|\/Users|\/private\/var\/folders|\/private\/tmp|\/tmp|\/var\/folders)\/[^\s"'`]+/g, '<PATH>')
    .replace(/\s+/g, ' ')
    .trim();
  return diagnostic === '' ? undefined : diagnostic.slice(0, 1_000);
}
...
export function validateFrozenCandidateSnapshot(snapshot, expectedVersion) {
  if (snapshot === null || typeof snapshot !== 'object' || Array.isArray(snapshot)) {
    throw new GuardianError('CANDIDATE_SNAPSHOT_INVALID', 'frozen candidate snapshot must be an object');
  }
  if (typeof snapshot.package !== 'string' || snapshot.package === '') {
    throw new GuardianError('CANDIDATE_SNAPSHOT_INVALID', 'frozen candidate snapshot omitted package');
  }
  if (typeof snapshot.version !== 'string' || !semver.valid(snapshot.version)) {
    throw new GuardianError('CANDIDATE_SNAPSHOT_INVALID', 'frozen candidate snapshot omitted an exact semantic version');
  }
  if (expectedVersion && snapshot.version !== expectedVersion) {
    throw new GuardianError('CANDIDATE_SNAPSHOT_MISMATCH', `frozen candidate ${snapshot.version} does not match requested ${expectedVersion}`);
  }
  if (typeof snapshot.integrity !== 'string' || snapshot.integrity === '') {
    throw new GuardianError('CANDIDATE_SNAPSHOT_INVALID', 'frozen candidate snapshot omitted package integrity');
  }
  if (typeof snapshot.graphDigest !== 'string' || !/^[0-9a-f]{64}$/i.test(snapshot.graphDigest)) {
    throw new GuardianError('CANDIDATE_SNAPSHOT_INVALID', 'frozen candidate snapshot omitted a valid install graph digest');
  }
...
async function waitForHttp(url, service, timeoutMs, headers = {}) {
...
function launchUrlFromOutput(output, baseUrl) {
  const expectedOrigin = new URL(baseUrl).origin;
  const matches = String(output ?? '').matchAll(/dsh web:\s+(https?:\/\/[^\s()]+)/g);
  for (const match of matches) {
    try {
      const url = new URL(match[1]);
      if (url.origin === expectedOrigin) return url;
    } catch {
      // Ignore unrelated or incomplete startup output while the service boots.
    }
  }
  return undefined;
}
...
export async function establishWebSession(baseUrl, service, timeoutMs) {
  const started = Date.now();
  let lastError;
  while (Date.now() - started < timeoutMs) {
    if (service.exited) throw new GuardianError('WEB_EXITED', 'dsh web exited before authentication', service.snapshot());
    // DSH 0.1.2 prints a one-time launch token. The ordinary service snapshot
    // deliberately redacts query tokens, so use the transient protocol buffer
    // only for the exchange and never persist it in verifier evidence.
    const protocolOutput = service.protocolStdout?.() ?? service.snapshot().stdout;
    const launchUrl = launchUrlFromOutput(protocolOutput, baseUrl);
    if (launchUrl !== undefined) {
      if (!launchUrl.searchParams.has('token')) return { cookie: undefined, durationMs: Date.now() - started };
      try {
        ...
        if (response.status >= 300 && response.status < 400 && cookie !== undefined) {
          return { cookie, durationMs: Date.now() - started };
        }
        lastError = new Error(`token exchange returned HTTP ${response.status}`);
      } catch (error) {
        lastError = error;
      }
    } else {
      try {
        ...
        if (response.status >= 200 && response.status < 300) {
          return { cookie: undefined, durationMs: Date.now() - started };
        }
        lastError = new Error(`waiting for authenticated startup URL after HTTP ${response.status}`);
      } catch (error) {
        lastError = error;
      }
    }
  ...
  throw new GuardianError('WEB_AUTH_TIMEOUT', `dsh web did not establish a browser session: ${lastError?.message ?? 'timeout'}`);
}
...
async function executeAssertion(baseUrl, assertion, cookie) {
  const response = await fetch(new URL(assertion.path, baseUrl), {
    method: assertion.method ?? 'GET',
    headers: { ...assertion.headers, ...(cookie === undefined ? {} : { cookie }) },
...
export async function verifyRepository(options) {
  ...
  try {
    ...
    if (options.candidateSnapshot !== undefined) {
      candidate = validateFrozenCandidateSnapshot(options.candidateSnapshot, options.dshVersion);
      steps.push({ name: 'reuse-frozen-candidate', ok: true, durationMs: 0 });
    } else if (config.watch.source === 'github-release') {
    ...
    // /private/var. Replace the longer alias first so reports never retain a
    // misleading "file:///private<GUARDIAN_TEMP>" prefix.
    if (temporaryRoot.startsWith('/var/')) replacements.push([`/private${temporaryRoot}`, '<GUARDIAN_TEMP>']);
    ...
    const candidatePackagePath = join(runnerDirectory, 'package.json');
    const candidateLockPath = join(runnerDirectory, 'pnpm-lock.yaml');
    let cachedPackage;
    let cachedLock;
    if (options.candidateGraphCacheDirectory) {
      try {
        [cachedPackage, cachedLock] = await Promise.all([
          readFile(join(options.candidateGraphCacheDirectory, 'package.json'), 'utf8'),
          readFile(join(options.candidateGraphCacheDirectory, 'pnpm-lock.yaml'), 'utf8'),
        ]);
      } catch (error) {
        if (error?.code !== 'ENOENT') throw error;
      }
    }
    if (cachedPackage !== undefined && cachedLock !== undefined) {
      await Promise.all([
        writeFile(candidatePackagePath, cachedPackage),
        writeFile(candidateLockPath, cachedLock),
      ]);
      ...
      await writeFile(candidatePackagePath, `${JSON.stringify({ private: true, type: 'module' }, null, 2)}\n`);
      await addCommandStep(
        'install-candidate-dsh',
        'pnpm',
    ...
    const graphLock = candidateLockPath;
    const expectedGraphDigest = candidate.graphDigest;
    const installedGraphDigest = await sha256File(graphLock);
    if (expectedGraphDigest && installedGraphDigest !== expectedGraphDigest) {
      throw new GuardianError(
        'CANDIDATE_GRAPH_DRIFT',
        `installed DSH dependency graph ${installedGraphDigest} does not match frozen ${expectedGraphDigest}`,
      );
    }
    candidate.graphDigest = installedGraphDigest;
    candidate.graphSource = 'pnpm-lock.yaml';
    if (options.candidateGraphCacheDirectory && cachedLock === undefined) {
      await mkdir(options.candidateGraphCacheDirectory, { recursive: true });
      await Promise.all([
        writeFile(join(options.candidateGraphCacheDirectory, 'package.json'), await readFile(candidatePackagePath, 'utf8')),
        writeFile(join(options.candidateGraphCacheDirectory, 'pnpm-lock.yaml'), await readFile(candidateLockPath, 'utf8')),
      ]);
    }
    ...
    service = startService(dshBinary, ['web', '--host', '127.0.0.1', '--port', String(port), '--no-open'], {
      cwd: repoPath,
      env: candidateEnv,
    });
    const webStarted = Date.now();
    const webTimeoutMs = (contract.web?.ready?.timeout_seconds ?? 120) * 1000;
    const webSession = await establishWebSession(baseUrl, service, webTimeoutMs);
    steps.push({ name: 'authenticate-real-dsh-web', ok: true, durationMs: webSession.durationMs });
    await waitForHttp(
      new URL(contract.web?.ready?.path ?? '/', baseUrl),
      service,
      Math.max(1_000, webTimeoutMs - webSession.durationMs),
      webSession.cookie === undefined ? {} : { cookie: webSession.cookie },
    );
    steps.push({ name: 'start-real-dsh-web', ok: true, durationMs: Date.now() - webStarted });

    const assertionStarted = Date.now();
    const assertionResults = [];
    for (const assertion of contract.web.assertions) assertionResults.push(await executeAssertion(baseUrl, assertion, webSession.cookie));
    ...
    const diagnostic = sanitizedVerifierDiagnostic(guardianError.details, replacements);
    report = {
      schema: 1,
      ...
      error: {
        ...
        message: sanitizeOutput(guardianError.message, replacements),
