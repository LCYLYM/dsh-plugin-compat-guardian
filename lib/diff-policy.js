...
export async function classifyRepairDiff(repoPath, changedPaths, numstat, config = {}) {
  const forceReview = [];
  const reject = [];
  const workspace = String(config.plugin?.workspace ?? '.').replaceAll('\\', '/').replace(/^\.\//, '').replace(/\/$/, '') || '.';
  const manifestRelative = posix.join(workspace, config.plugin?.package_json ?? 'package.json');
  const sourceRoot = workspace === '.' ? 'src' : `${workspace}/src`;
  const outputRoots = ['lib', 'dist'].map(name => workspace === '.' ? name : `${workspace}/${name}`);
  ...
  const lockChanges = status.trim().split('\n').filter(Boolean)
    ...
    .filter(item => LOCKFILES.has(item.path.split('/').at(-1)));
  if (lockChanges.some(item => /^[AD]/.test(item.state))) reject.push('package manager lockfile added or removed');

  const after = JSON.parse(await readFile(resolve(repoPath, manifestRelative), 'utf8'));
  if (after.scripts?.build && changedPaths.some(path => outputRoots.some(root => path.startsWith(`${root}/`)))) {
    const trackedSource = (await runCommand('git', ['ls-files', sourceRoot], { cwd: repoPath, timeoutMs: 30_000 })).stdout.trim();
    if (trackedSource !== '' && !changedPaths.some(path => path.startsWith(`${sourceRoot}/`))) {
      reject.push('tracked build output changed without its source');
    }
  }
  if (changedPaths.includes(manifestRelative)) {
    let before;
    try {
      const text = (await runCommand('git', ['show', `HEAD:${manifestRelative}`], { cwd: repoPath, timeoutMs: 30_000 })).stdout;
      before = JSON.parse(text);
    } catch (error) {
      throw new GuardianError('DIFF_POLICY_INVALID', `cannot read base ${manifestRelative}: ${error.message}`);
    }
