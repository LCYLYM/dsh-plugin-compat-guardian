...
export function packageManagerInstallArgs(packageManager) {
  ...
  if (packageManager.name === 'npm') return hasLock ? ['ci'] : ['install'];
  if (packageManager.name === 'pnpm') return hasLock ? ['install', '--frozen-lockfile'] : ['install'];
  if (packageManager.name === 'yarn') {
    const major = Number(packageManager.exactVersion.split('.')[0]);
    return hasLock ? ['install', major >= 2 ? '--immutable' : '--frozen-lockfile'] : ['install'];
  }
