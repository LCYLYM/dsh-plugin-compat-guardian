...
export function packageManagerInstallArgs(packageManager) {
  const hasLock = packageManager.lockfiles.length > 0;
  if (packageManager.name === 'npm') return hasLock ? ['ci'] : ['install', '--no-package-lock'];
  if (packageManager.name === 'pnpm') return hasLock ? ['install', '--frozen-lockfile'] : ['install', '--lockfile=false'];
  if (packageManager.name === 'yarn') {
    const major = Number(packageManager.exactVersion.split('.')[0]);
    return hasLock ? ['install', major >= 2 ? '--immutable' : '--frozen-lockfile'] : ['install', '--no-lockfile'];
  }
  throw new GuardianError('BLOCKED_UNSUPPORTED', `package manager ${packageManager.name} is not supported in V1`);
}
