...
async function discoverContractWithDsh(repoPath, config, hints) {
  ...
  try {
    ...
    await writeFile(overlayPath, stringify([{ id: 'agent-default-model', config: { provider: config.repair.provider, model: config.repair.model } }]));
    ...
    const result = await runCommand(dsh, ['--profile', 'headless', '--patch', overlayPath, prompt], {
      ...
      env: {
        ...process.env, CI: 'true', DSH_HOME: dshHome, DSH_PERMISSION_MODE: 'read-only',
