...
const NOTIFIABLE = new Set(['WAITING_FOR_PRICE', 'WAITING_FOR_NPM_ARTIFACT', 'CONVERGE_30', 'PASS', 'BLOCKED', 'BLOCKED_EXTERNAL', 'SUPERSEDED']);
...
export async function notifyFromReport(options) {
  ...
  for (const event of events) {
    ...
    let comments;
    try {
      issue = await campaignIssue(event, ghEnv);
      comments = JSON.parse((await runCommand('gh', ['api', `repos/${process.env.GITHUB_REPOSITORY}/issues/${issue.number}/comments`, '--paginate'], {
        timeoutMs: 60_000, env: ghEnv,
      })).stdout || '[]');
    } catch {
      // GitHub may restrict Issues for forks or GITHUB_TOKEN even when the
      // compatibility publisher itself can write. Keep the Actions summary
      // authoritative and still deliver explicitly configured side channels.
      delivered.push({
        event,
        adapters: await sendConfiguredAdapters(event, config),
        githubIssue: 'UNAVAILABLE',
      });
      continue;
    }
