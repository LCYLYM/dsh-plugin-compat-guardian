...
const STATUS_TEXT = Object.freeze({
  ...
  NOOP: '💤 无需处理',
  BLOCKED: '❌ 已阻塞',
  BLOCKED_CONFIG: '🛠️ 配置或凭据需要修正',
  BLOCKED_EXTERNAL: '⏸️ 外部服务暂不可用',
  BLOCKED_CONTRACT: '⚠️ 需要人工审核测试契约',
  WAITING_FOR_NPM_ARTIFACT: '📦 等待 NPM 制品发布',
  FROZEN: '🔒 已冻结',
...
const STEP_TEXT = Object.freeze({
  ...
  'resolve-github-release': '冻结官方 GitHub Release 候选',
...
function nextAction(report) {
  ...
  if (report.status === 'WAITING_FOR_NPM_ARTIFACT') return '官方 GitHub Release 已发现，但对应 NPM 制品尚未就绪；Guardian 不会调用模型或创建 PR，下一次轮询会继续检查。';
  if (report.status === 'BLOCKED_EXTERNAL') return '代码尚未判定为不兼容。外部服务恢复后手工重跑，schedule 不会循环烧额度。';
  if (report.status === 'BLOCKED_CONFIG') return '请按下方错误代码修正 Secret、base URL、model 或 provider，然后手工重跑。修正前 schedule 不会反复调模型。';
  if (report.status === 'BLOCKED_CONTRACT') return '只审核独立的 contract PR；合并后会全量复测，已消耗预算不会重置。';
  if (report.status === 'FROZEN') return report.error?.code === 'STATE_PUBLICATION_PENDING' || report.error?.code === 'MODEL_RECOVERY_SIGNAL_REQUIRED'
    ? '本版本已有等待处理的状态/维修记录，或正等待明确恢复信号；定时任务不会再调模型。'
    : '本版本已停止自动继续。只有提高额度或把 `resetBudget` 从 `N` 改为 `Y` 后才会再修一次。';
...
export function renderMarkdown(report) {
  ...
  const lines = [
    ...
    ...(report.candidate?.release ? [
      `| 官方 Release | [\`${report.candidate.release.tag}\`](${report.candidate.release.url ?? '#'}) |`,
      `| Release commit | \`${report.candidate.release.commit?.slice(0, 12) ?? '未知'}\` |`,
    ] : []),
  ...
  if (report.error) {
    ...
    const diagnostic = report.error.diagnostic ?? firstFailed?.diagnostic;
    if (diagnostic) lines.push(`- 去敏诊断：\`${diagnostic.replaceAll('`', "'")}\``);
  }
  if (report.budget) {
    ...
    lines.push(
      ...
      `| Guardian 维修轮次 | ${report.budget.state?.consumed?.attempts ?? 0} / ${report.budget.state?.ratios?.attempts === null ? '未设置' : limits.max_attempts ?? '见 repair.max_attempts'} |`,
      '',
      '> 费用是按 DSH 已报告 usage 估算，不是 DeepSeek 账户账单级实时扣费。',
      '> Guardian 维修轮次是 DSH 维修进程轮次，不是 provider 底层 HTTP 请求数；DSH 一轮可能发出多个 API 请求。',
    );
  }
  if (report.repair) {
    lines.push(
