...
const RULES = [
  ...
  { code: 'MODEL_REQUEST_INVALID', status: 'BLOCKED_CONFIG', retryable: false, pattern: /INVALID_REQUEST|request is invalid|max_tokens[^\n]{0,100}(?:invalid|非法|range|范围)|(?:invalid|非法)[^\n]{0,100}max_tokens/i, message: '模型服务拒绝了请求参数。请检查该路由的单次输出上限等兼容设置后手工重跑。' },
  { code: 'MODEL_RATE_LIMITED', status: 'BLOCKED_EXTERNAL', retryable: true, pattern: /\b429\b|rate[_ -]?limit|too many requests/i, message: '模型服务限流（429），每两分钟自动重试一次，最多再试两次；仍失败则冻结定时重跑。' },
  { code: 'MODEL_PROVIDER_TIMEOUT', status: 'BLOCKED_EXTERNAL', retryable: true, pattern: /timed?\s*out|timeout|ETIMEDOUT|AbortError/i, message: '模型服务请求超时，每两分钟自动重试一次，最多再试两次；仍失败则冻结定时重跑。' },
  { code: 'MODEL_PROVIDER_5XX', status: 'BLOCKED_EXTERNAL', retryable: true, pattern: /(?:HTTP|status|code)?\s*5\d\d\b|internal server error|bad gateway|service unavailable|gateway timeout|(?:system|server|service)[^\n]{0,40}(?:cpu\s+)?overloaded|server (?:is )?(?:busy|overloaded)|high server load/i, message: '模型服务返回 5xx 或明确报告过载，每两分钟自动重试一次，最多再试两次；仍失败则冻结定时重跑。' },
