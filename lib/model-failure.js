...
const RULES = [
  ...
  { code: 'MODEL_RATE_LIMITED', status: 'BLOCKED_EXTERNAL', retryable: true, pattern: /\b429\b|rate[_ -]?limit|too many requests/i, message: '模型服务限流（429），等待一分钟后最多重试一次；仍失败则冻结定时重跑。' },
  { code: 'MODEL_PROVIDER_TIMEOUT', status: 'BLOCKED_EXTERNAL', retryable: true, pattern: /timed?\s*out|timeout|ETIMEDOUT|AbortError/i, message: '模型服务请求超时，等待一分钟后最多重试一次；仍失败则冻结定时重跑。' },
  { code: 'MODEL_PROVIDER_5XX', status: 'BLOCKED_EXTERNAL', retryable: true, pattern: /(?:HTTP|status|code)?\s*5\d\d\b|internal server error|bad gateway|service unavailable|gateway timeout/i, message: '模型服务返回 5xx，等待一分钟后最多重试一次；仍失败则冻结定时重跑。' },
