...
export function githubRequestHeaders(url, env = process.env) {
  const token = env.GITHUB_TOKEN ?? env.GH_TOKEN;
  if (!token) return {};
  try {
    const trustedApi = new URL(env.GITHUB_API_URL ?? 'https://api.github.com');
    const requested = new URL(url);
    if (requested.origin !== trustedApi.origin) return {};
  } catch {
    return {};
  }
  return {
...
async function fetchJson(url, timeoutMs, headers = {}) {
  ...
  try {
    response = await fetch(url, {
      ...
      headers: { accept: 'application/vnd.github+json', ...githubRequestHeaders(url), ...headers },
    });
  } catch (error) {
