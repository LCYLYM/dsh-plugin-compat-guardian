...
export function repairCampaignGate({
  ...
  if (['BLOCKED_CONFIG', 'BLOCKED_EXTERNAL'].includes(campaign?.status)
    && !['workflow_dispatch', 'push'].includes(trigger)) {
    return { status: 'FROZEN', reason: 'model-recovery-signal-required', reset: null };
  }
...
export function modelInputUnchangedAfterPush(previous, currentFingerprint, trigger) {
  return String(previous?.status ?? '').startsWith('BLOCKED')
    && trigger === 'push'
    && typeof previous?.input_fingerprint === 'string'
    && previous.input_fingerprint === currentFingerprint;
}
