...
- Judge a contract-required real model smoke only from fixed inputs and repeatable mechanical evidence: plugin input handling, required attachment/image presence in the outgoing request, provider success, and a non-empty result consumed by DSH. Never match exact answer wording or subjective quality. User-reviewed deterministic assertions may be stronger; the repair agent cannot edit them.
