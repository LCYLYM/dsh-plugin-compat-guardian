...
日期：2026-08-22
执行状态：M0–M3 代码完成；主链路已公开真实 PASS，可选外部投递与需真实上游事件的组合保持 partial。
...
M0 作为无模型基础切片保持可独立运行；repair/model、三种交付模式、预算与通知已在 M2/M3 接入，不改变 M0 默认无 key 的边界。
...
| R6 | M2 | campaign ledger、usage 归一化、CNY map、30% steer | 跨 run 累计与四个硬上限故障注入 | pass |
| R7 | M1/M2 | 状态机、concurrency、一次自动维修门 | schedule 重放不再调模型 | pass |
...
| R11 | M1/M3 | publisher 的 PR/auto-merge/direct-push 三模式 | 三模式 GitHub 回读及保护分支失败 | partial |
| R12 | M2 | `N -> Y` edge、budget epoch、reset commit 去重 | 静态 Y/rerun 不重复恢复 | pass |
| R13 | M1 | current-repo-only guard；不读取 upstream remote | fork fixture 零 upstream 写调用 | pass |
| R14 | M1/M3 | Summary/Issue + email/TG/webhook 窄 adapter | 终态消息真实回读且去重 | partial |
| R15 | M2/M3 | 默认 vision model、真实图片 smoke、DSH search 事件 | image 入请求、搜索调用/usage 分开记录 | pass |
| R16 | M2 | 价格 revision、时区窗口、立即检测/延迟维修 | 峰谷边界和未知 route CNY unknown | pass |
| R17 | M0/M2 | 同根 graph 变化复测；不赠预算 | 两份 graph、完整 gate、repair 门保持 | pass |
| R18 | M0/M2 | 先无 key PASS，再注入 `httpServer` 故障真修 | 两阶段公开证据，均无自动合并 | pass |
| R19 | M2 | realpath denylist、protected diff guard | 普通源码允许；每类保护路径拒绝 | pass |
| R20 | M2 | 测试面 classifier 强制普通 PR | test/config/script 三类降级 | pass |
| R21 | M2 | 依赖 diff classifier 和因果说明 | allow/reject/human-review 三组样例 | pass |
| R22 | M2 | build/source detector、干净重建、byte compare | deterministic/nondeterministic/no-build 四样例 | partial |
| R23 | M2 | 普通文件增删；复用同一保护清单 | 普通删除通过、保护删除拒绝、误删 gate fail | pass |
| R24 | M2 | diff stat 只报告不设阈值 | 大生成 diff 不误拦、预算仍生效 | pass |
| R25 | M1/M2 | event/ref/permission guard；分 job secret 映射 | 三允许事件与 PR/fork/ref 拒绝矩阵 | pass |
| R26 | M2 | baseline/candidate failure classifier | DSH 差分才 repair，无关失败只报告 | pass |
| R27 | M1 | `17 */6 * * *`、manual、控制文件 push filter | 四触发、源码 push 静默、latest 跳变 | pass |
| R28 | M0 | 无 verified 时先跑 frozen repair baseline | baseline PASS/FAIL/与 latest 同 snapshot 去重 | pass |
| R29 | M1/M2 | base SHA 二次读取、STALE_SOURCE | 四阶段竞态测试且预算不重置 | pass |
| R30 | M1/M2 | target-version branch/PR、supersede close | 旧 PR 留痕关闭、新 PR 新建、无 force push | partial |
| R31 | M1 | full-SHA workflow ref validator/generator | 拒绝 branch/tag；DSH 更新 workflow 零 diff | pass |
| R32 | M0/M1 | no-repair PASS 只改 lock，报告 URL 持久化 | 三交付模式、下一轮 NOOP | pass |
| R33 | M2 | token/CNY/time/attempt 硬门和一次 steer | 四上限、等待不计时、尾差阻断 | pass |
| R34 | M2 | repair prompt + DSH native search telemetry | 用/不用/失败/无 fallback 四记录 | partial |
| R35 | M3 | reviewed contract 才给 candidate smoke Secret | 默认无 key、缺 key blocked、usage 入同桶 | pass |
...
| R39 | M3 | timeout/429/5xx/overload two-minute bounded backoff + frozen external state | 同一回合最多再试两次；schedule NOOP、manual/config/new target 恢复 | pass |
...
| R43 | M3 | external recovery edge，不改 repair ledger | manual/config 恢复与 schedule NOOP | pass |
| R44 | M1 | lock 仅机器状态+URL；报告在 GitHub surface | 三交付模式，仓库无版本报告文件 | pass |
| R45 | M2 | publisher squash/clean commit，失败尝试不发布 | 两轮后仍只有一个 bot commit | partial |
| R46 | M1/M3 | Summary always + event-id notifier dedupe | 所有允许事件一次，NOOP 零通知 | partial |
| R47 | M0/M1 | Node 声明优先级、冲突检查、setup-node 接线 | 三声明/fallback/conflict/精确版本 | pass |
| R48 | M0/M1 | packageManager/lockfile resolver，npm/pnpm/yarn | 三支持/fallback/conflict/Bun blocked | pass |
| R49 | M1 | 默认 `ubuntu-24.04` + 单 label override | workflow 无 matrix 且报告 actual OS | pass |
| R50 | M0/M1 | onboard 临时分支、gh PR/fallback、secret scan | 有/无 gh 两条真实路径，默认分支零 diff | pass |
| R51 | M1/M3 | GITHUB_TOKEN publisher、WAITING_APPROVAL、可选 PAT | 默认 PR、审批等待、token job 隔离 | partial |
...
- [x] 所有 `ACCEPTANCE.md` ID 保持不变，状态由证据更新而非口头更新。
- [x] `npm test`、CLI smoke、fixture real run 全部 PASS。
- [x] `git diff --check` 和 secret/path/transcript 扫描通过。
- [x] README 对 auto-merge/direct-push、candidate model key 和价格估算风险说人话。
- [x] workflow 引用完整 SHA，权限逐 job 最小化。
- [x] 失败不会更新 verified，不会调用 publisher，不会静默 fallback。
- [x] 公开 fixture 的 Actions/PR/lock URL 回读完成。
- [x] M2 模型维修前已确认预算、Secret 名和低价策略。
