...
| D41 | 真实模型 smoke 遇到 timeout、429、provider 5xx 或明确过载时，在同一回合内每两分钟自动重试一次、最多再试两次；仍失败则为 `BLOCKED_EXTERNAL`，不启动 repair，也不由六小时 schedule 反复重试。手工运行、相关配置变更或新目标版本才是下一次信号。 |
...
9. 模型 smoke 的 timeout、429、provider 5xx 或明确过载在同一回合内每两分钟自动重试一次、最多再试两次；仍失败即冻结为 `BLOCKED_EXTERNAL`，schedule 不得循环调用。
...
Guardian 为 `llm-deepseek` 设置 `retryPolicy.maxRetries: 2` 和固定两分钟退避，并禁用与兼容结论无关的 LLM 会话标题生成，避免计费外请求。这意味着 timeout/429/5xx/明确过载的主模型请求最多会在两分钟、四分钟时各再试一次；`attempts_used` 仍表示 Guardian 维修轮次，不是 HTTP 请求计数。
